...
function isPrime(n) {
  ...
  if (n < 2) {
    ...
    const dev = Math.floor(Math.sqrt(n)) + 1;
    for (let i = 2; i <= dev; i += 1) {
      if (n % i === 0 && i !== n) {
        ans = false;
        break;
      } else {
...
function getSumOfDigits(num) {
  return [...String(num)].reduce((prev, current) => +prev + +current);
}
...
function isNumber(number) {
  let ans;
  if (typeof number === 'number' && Number.isFinite(number)) ans = true;
  else ans = false;
  return ans;
}
...
function getFloatOnString(str) {
  let ans;
  if (!Number.isNaN(Number.parseFloat(str))) ans = Number.parseFloat(str);
  else ans = NaN;
  return ans;
}
...
function getSumOfNumbers(...rest) {
  return [...rest].reduce((prev, current) => prev + current).toFixed(2);
}
